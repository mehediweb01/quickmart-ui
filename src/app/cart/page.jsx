...
const Cart = () => {
  ...
  return (
    <div className="min-h-screen py-16 mx-8">
      ...
      <div>
        <div className="sm:flex hidden justify-between items-center gap-4 mt-8 ">
          {heading.map((heading, i) => (
            <h1 key={i} className="font-poppins font-medium text-black text-xl">
              {heading}
            </h1>
          ))}
        </div>
        <div className="space-y-3 my-8">
          {cartItems &&
            cartItems.map((item) => <ProductItem key={item.id} item={item} />)}
        </div>
        <div className="w-full py-16">
          <div className="flex md:flex-row flex-col gap-4 items-start float-end">
            <div>
